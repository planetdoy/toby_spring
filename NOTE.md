...
### 예외를 다루는 방법
![예외](./IMG/6_45_1.jpg)
![예외가 발생하면](./IMG/6_45_2.jpg)
![예외를 잘못 다루는 코드1](./IMG/6_45_3.jpg)
...
![예외의 종류](./IMG/6_45_5.jpg)
![Error](./IMG/6_45_6.jpg)
![체크 예외](./IMG/6_45_7.jpg)
> 런타임 예외를 상속하지 않은 것  
> 컴파일이 안됨
...
> ![JDBC SQLException](./IMG/6_48_1.jpg)  
> ![DataAccessException_1](./IMG/6_48_2.jpg)  

> ![DataAccessException_2](./IMG/6_48_3.jpg)  
...
![스프링 애플리케이션의 빈이 존재하는 계층 구조](./IMG/7_49_2.jpg)  

![service x](./IMG/7_49_3.jpg)  

![서비스는 일반적인 용어](./IMG/7_49_4.jpg)

![서비스는 클라이언트에게 ...](./IMG/7_49_5.jpg)

![서비스는 일반적으로 ...](./IMG/7_49_6.jpg)

> 서버에 존재하는 서비스는 크게 3가지가 된다.  
![서비스 종류](./IMG/7_49_7.jpg)  
...
![애플리케이션 서비스 설명 2](./IMG/7_50_2.jpg)  
...
> 그림과 같이 오토커밋을 정지시킬 수 있다.
![JDBC 트랜잭션](./IMG/7_53_4.jpg)  

![추상화](./IMG/7_53_5.jpg)

![개별 트랜잭션 기술의 디테일에 의존하는 클라이언트 1](./IMG/7_53_6.jpg)

![개별 트랜잭션 기술의 디테일에 의존하는 클라이언트 2](./IMG/7_53_7.jpg)

![트랜잭션을 관리하는 오브젝트의 추상화 1](./IMG/7_53_8.jpg)


> 사용하는 기술에 따라 클라이언트가 변경이 되지 않도록 디테일에 의존하는 방식이라면 기술이 변경되면 코드가 크게 변경되어야한다.  
> 이는 많은 문제들을 발생시킬 수 있다.  

> 그러면 추상화를 한다. 공통적인 기술을 추상화한다.  
> 특정 기술에 종속되지 않은 공통적인 추사화된 메서드  
> 인터페이스는 정의하고 이를 사용하는 클라이언트는 해당 인터페이스만 의존하도록 하면 해결이 될 것 같지만  
> 인터페이스를 사용하도록 해당 기술들을 변경할 수는 없기 때문에 이를 위해 중간에 변환해주는 어댑터를 사용한다.  
> *어댑터 : 중간에 각 기술을 사용하는 코드이면서 추상화 된 인터페이스를 구현한 것  
![트랜잭션을 관리하는 오브젝트의 추상화 2](./IMG/7_53_9.jpg)

### JDBC 데이터 액세스 기술
![JdbcClient](./IMG/7_54_1.jpg)    

![DataSourceTransactionManager](./IMG/7_54_2.jpg)  

![JDBC 데이터 액세스용 구성 정보](./IMG/7_54_3.jpg)  

> 추상화를 잘 적용하면 실제 사용하는 서비스에  
> 코드를 변경하지 않고 사용하는 기술을 변경할 수 있다.      

> JPA에서 JDBC로 기술을 변경하면서 OrderService의 코드는  
> 변경하지 않고 Config 파일의 코드만 변경되었다.  

> 표준구현, 오픈소스 여러가지의 구현체가 나온 기술을 적용할 때  
> 공통적으로 액션이 같거나 제공하는게 같다면    
> 서비스 추상화를 적용해서 기술을 적용하는게 좋다.  
